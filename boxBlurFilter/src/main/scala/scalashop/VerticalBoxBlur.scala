...
object VerticalBoxBlur extends VerticalBoxBlurInterface:
  ...
  def parBlur(src: Img, dst: Img, numTasks: Int, radius: Int): Unit =
    ...
    val remPixels = src.width % numTasks

    val step = if (src.width > numTasks) then src.width/numTasks else 1
    var splitRange = 0 to (if step == 1 then src.width else (src.width - remPixels)) by step
