...
class KMeans extends KMeansInterface:
  ...
  def classify(points: ParSeq[Point], means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] =
    ...
    else if points.isEmpty then 
      ParMap(means.head -> points)
    else points.groupBy(findClosest(_, means.seq))
  ...
  def update(classified: ParMap[Point, ParSeq[Point]], oldMeans: ParSeq[Point]): ParSeq[Point] =
    val oldMeansIter = oldMeans.iterator
    val classifiedIter = classified.values.iterator
    var newMeans = ParSeq[Point]()
    while(oldMeansIter.hasNext)
      newMeans = newMeans :+ findAverage(oldMeansIter.next, classifiedIter.next)
    newMeans


  def converged(eta: Double, oldMeans: ParSeq[Point], newMeans: ParSeq[Point]): Boolean =
    oldMeans.corresponds(newMeans)((x,y) => x.squareDistance(y) <= eta)

  @tailrec
  final def kMeans(points: ParSeq[Point], means: ParSeq[Point], eta: Double): ParSeq[Point] =
    val classified = classify(points, means)
    val oldMeans = classified.keys.toSeq
    val newMeans = update(classified, oldMeans)
    if (!converged(eta, oldMeans, newMeans)) kMeans(points, newMeans, eta) else newMeans
...
object KMeansRunner:
  ...
  def main(args: Array[String]): Unit =
    ...
    val seqtime =
      ...
      // Creates a support with only one executor to run the algorithm sequentially
